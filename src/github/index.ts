  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description: "List workflow runs for a GitHub repository",
      ...
      {
        name: "list_workflow_runs_by_workflow_id",
        description: "List workflow runs for a specific workflow ID",
      ...
      },
      {
        ...
        description: "Download a file from a URL and unzip it to a specified directory (defaults to /tmp)",
        inputSchema: zodToJsonSchema(downloader.DownloadAndUnzipSchema),
      },
      {
  ...
  try {
    ...
    switch (request.params.name) {
      ...
      case "list_workflow_runs": {
        ...
        const results = await workflows.listWorkflowRuns(args);
        return {
          content: [{ type: "text", text: JSON.stringify(results, null, 2) }],
        };
      }
      ...
      case "get_workflow_run": {
        ...
        const result = await workflows.getWorkflowRun(args);
        return {
          content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        };
      }

      case "list_workflow_runs_by_workflow_id": {
        const args = workflows.ListWorkflowRunsByWorkflowIdSchema.parse(request.params.arguments);
        const results = await workflows.listWorkflowRunsByWorkflowId(args);
        return {
          content: [{ type: "text", text: JSON.stringify(results, null, 2) }],
        };
      }
      ...
      case "get_workflow_run_logs": {
        const args = workflows.GetWorkflowRunLogsSchema.parse(request.params.arguments);
        const result = await workflows.getWorkflowRunLogs(args);
        return {
          content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        };
      }
      ...
      case "download_and_unzip": {
        const args = downloader.DownloadAndUnzipSchema.parse(request.params.arguments);
        const result = await downloader.downloadAndUnzip(args);
        return {
          content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        };
      }
      
      case "analyze_workflow_logs": {
        const args = logAnalyzer.AnalyzeWorkflowLogsSchema.parse(request.params.arguments);
